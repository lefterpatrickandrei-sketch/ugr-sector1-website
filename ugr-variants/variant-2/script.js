// State-ul aplicației
        ...
        // Colors: brass gold (#C3905C) for branches, terracotta orange (#E76F51) for active HQ
        // ══════════════════════════════════════════════════════════════════
        const mapNodes = {
        ...
        // ══════════════════════════════════════════════════════════════════
        // GLOBE.GL — Surveyor's Brass Theme (Gold/Terracotta Theme)
        // ══════════════════════════════════════════════════════════════════
        let myGlobe;
        ...
        function initGlobeMap() {
            ...
            myGlobe = Globe()(container)
                ...
                .showAtmosphere(true)
                .atmosphereColor('#C3905C') // Brass Gold
                .atmosphereAltitude(0.15)
                .showGraticules(true) // grid lines on the globe
                .width(w)
                .height(h);

            // Style the globe shell to look like dark translucent brass-tinted glass
            myGlobe.globeMaterial().color.set('#0b0a06');

            // Set points data - HQ in terracotta, branches in brass gold
            const gData = Object.keys(mapNodes).map(key => ({
                ...
                color: key === 'bucuresti' ? '#E76F51' : '#C3905C'
            }));
            ...
            const initialArcs = Object.keys(mapNodes)
                ...
                .map(key => ({
                    ...
                    color: '#C3905C' // Brass connection line
                }));
            ...
            myGlobe
                ...
                .polygonStrokeColor(d => {
                    const admin = d.properties.ADMIN;
                    if (admin === 'Romania' || admin === 'Moldova') {
                        return 'rgba(195, 144, 92, 0.80)';
                    }
                    return 'rgba(195, 144, 92, 0.08)';
                })
                .polygonAltitude(d => {
                    const admin = d.properties.ADMIN;
                    if (admin === 'Romania' || admin === 'Moldova') {
        ...
        function filterMapBySkill() {
            ...
            const updatedPoints = Object.keys(mapNodes).map(key => {
                ...
                return {
                    ...
                    color: isHQ 
                        ? (isActive ? '#E76F51' : 'rgba(231, 111, 81, 0.4)') 
                        : (isActive ? '#C3905C' : 'rgba(195, 144, 92, 0.25)')
                };
            });
